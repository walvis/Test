DROP FUNCTION IF EXISTS walvis_upsert_sranch(int[], text[]);
...
--  inserted: 1
--
CREATE OR REPLACE FUNCTION walvis_upsert_sranch(
  cartodb_ids integer[],
  geojsons text[])
...
sql := sql || '), do_update AS ('
      || 'UPDATE table_4sranch_geocode_3_11_2016_xy p '
      ...
      || 'DELETE FROM table_4sranch_geocode_3_11_2016_xy p WHERE p.cartodb_id IN ('
      || 'SELECT n.cartodb_id FROM n WHERE cartodb_id >= 0 AND '
      || ' n.the_geom IS NULL ) RETURNING p.cartodb_id ), do_insert AS ('
      || 'INSERT INTO table_4sranch_geocode_3_11_2016_xy (the_geom)'
...
GRANT EXECUTE ON FUNCTION walvis_upsert_sranch(integer[],text[]) TO publicuser;